...
internal static class Program
{
    public static int Main(string[] args)
    {
        ...
        var sleepSecondsOption = new Option<int>("--sleep-seconds") { IsRequired = false };
        ...
        if (commandLine.GetValueForOption(sleepSecondsOption) is var sleep and > 0)
        {
            Console.WriteLine($"Waiting for {sleep} seconds...");
            Thread.Sleep(sleep * 1000);
        }

        if (commandLine.GetValueForOption(exitCodeOption) is var exitCode && exitCode != 0)
        {
