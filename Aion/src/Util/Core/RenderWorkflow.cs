...
using Aion.Util.Core.Templates;
...
)
{
    ...
    )
    {
        ...
        try
        {
            ...
            {
                Logging = await profile.GetLoggingOrDefault(template.Logging).Let(jsonObject => TemplateRendering.RenderFilePaths(jsonObject, variables)),
