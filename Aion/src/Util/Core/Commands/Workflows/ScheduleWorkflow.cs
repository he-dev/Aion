...
namespace Aion.Util.Core.Commands.Workflows;
    ...
    )
    {
        var trigger =
            startAtUtc is not null
                ? WorkflowTrigger.Create(workflowPath.ProfileName, workflowPath.WorkflowName, startAtUtc.Value)
                : null;

        var workflow = await renderWorkflow.For(workflowPath, trigger, stepOrder, loadTemplate);
