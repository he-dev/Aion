...
public class WorkflowTest
{
    ...
    {
        await Assert.ThrowsAsync<FileNotFoundException>(() => WorkflowConfiguration.FromFile(@"C:\fake\path\to", "workflow.json"));
    }
    ...
    {
        var ex = await Assert.ThrowsAsync<Scriban.Syntax.ScriptRuntimeException>(() => WorkflowConfiguration.FromFile(@"workflows", @"\invalid\uses-undefined-vars.json"));
    ...
    {
        var ex = await Assert.ThrowsAsync<Scriban.Syntax.ScriptRuntimeException>(() => WorkflowConfiguration.FromFile(@"workflows", @"\invalid\uses-undefined-env.json"));
    ...
    {
        await Assert.ThrowsAsync<FormatException>(() => WorkflowConfiguration.FromFile(@"workflows", @"\invalid\uses-invalid-cron.json"));
    }

    [Fact]
    public async Task CanReadFromJson()
    {
        var workflow = await WorkflowConfiguration.FromFile(@"workflows", @"\says-hallo.json");
